  ...
  describe JobsParse do
    ...
    describe "#mode_h" do
      it "generally works" do
        ...
        r = newo(auto_start: nil)
        expect(r.mode_h).to eq({})
        r = newo(auto_start: false)
        expect(r.mode_h).to eq({auto_start: false})
        r = newo(auto_start: true)
        expect(r.mode_h).to eq({auto_start: true})
      end
    end

    describe ".response_klass" do
      it { expect(described_class.response_klass).to eq Response::JobsParse }
    end
